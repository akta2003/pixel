...
def submit_mint(web3, wallet, contract, nonce: int) -> bool:
    try:
        ...
        # 1) Cek ulang POW sebelum transaksi dibuat.
        try:
            valid = contract.functions.isValidPow(wallet.address, nonce).call()
            if not valid:
                print("  ⚠️  POW nonce tidak valid / kemungkinan duplicate, skip TX.")
                return False
        except Exception as exc:
            print(f"  ⚠️  Validasi POW gagal: {exc}")
            return False

        # 2) Estimate gas. Kalau estimate revert, JANGAN kirim TX.
        try:
            gas_estimate = fn.estimate_gas({"from": wallet.address})
            gas_limit = int(gas_estimate * 1.30)
        except Exception as exc:
            err = str(exc)

            if "Duplicate POW nonce" in err:
                print("  ⚠️  Duplicate POW nonce, skip TX dan mining ulang...")
                return False

            if "execution reverted" in err:
                print(f"  ⚠️  TX diprediksi revert, skip TX: {exc}")
                return False

            print(f"  ⚠️  Estimate gas gagal, skip TX agar tidak rugi gas: {exc}")
            return False

        # 3) Pakai EIP-1559 gas dan pending nonce agar tidak bentrok dengan TX pending.
